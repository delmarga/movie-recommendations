...
def gradient(X, *args):
    ...
    P_grad = np.dot((mask * (np.dot(P, Q) - ratings)), Q.T) + \
        (regularization_amount * P)
    Q_grad = np.dot((mask * (np.dot(P, Q) - ratings)).T, P) + \
        (regularization_amount * Q.T)
...
def low_rank_matrix_factorization(ratings, mask=None, num_features=15, regularization_amount=0.01):
    ...
    args = (num_users, num_products, num_features,
